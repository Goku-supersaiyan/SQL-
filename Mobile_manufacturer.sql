...
IDModel INT PRIMARY KEY Auto_increment   , -- IDENTITY(101, 1),
...
IDCustomer INT PRIMARY KEY auto_increment , -- IDENTITY(10001, 1),
...
YEAR_ YEAR(_Date),
QUARTER_ AS DATEPART(QUARTER, [DATE]),
[MONTH] AS MONTH([DATE])
);*/
...
INSERT INTO DIM_MODEL (Model_name,Unit_price,IDManufacturer) VALUES
 -- 'iPhone 4', 377, '11'), ('iPhone 4S', 414, '11'),
...
where DTE between '2015-01-01' AND CURDATE() ;	
...
select state , SUM(QuanTitY) from fact_transaction a
...
select Manufacturer_Name from dim_model a
inner join dim_manufacturer b on a.IDManufacturer = b.IDManufacturer
where Manufacturer_Name = 'Samsung' ; 

-- 3. Show the number of transactions for each model per zip code per state.

select Distinct(Model_name )  , ZipCode , Count(b.IDModel) as Number_Of_Trans 
from Dim_model a
inner join Fact_transaction b on a.IDModel=b.IDModel 
inner join Dim_Location c on b.IDLocation = c.IDLocation 
group by Model_name , ZipCode
order by Count(b.IDModel) desc ; 
...
with xyz as 
(select manufacturer_name, totalprice, Model_name   , row_number() over (order by SUM(quantity) desc) as RNK 
from Dim_model a
inner join Dim_manufacturer b on a.IDManufacturer = b.IDManufacturer 
inner join fact_transaction c on a.IDModel = c.IDModel
group by Manufacturer_Name ,  Model_name , totalprice )
 
select Model_name, AVG(totalprice) from xyz where RNK <= 5 
group by Model_name ; 
...
from dim_Customer a
LEFT join fact_transaction b on a.IDCUSTOMER = b.IDCUSTOMER 
Where Year(DTE) = '2009'
group by  Customer_name 
Having AVG(totalprice)  >= 500 ; -- We see that there are no customers with AVG Price more than 500

-- Query to check the answer : 
Select IDcustomer,Year(DTE) ,  AVG(Totalprice) from Fact_transaction
Where Year(DTE) ='2009'
group by IDcustomer,Year(DTE) ;
...
with xyz as 
(Select Year(DTE), Model_name ,SUM(quantity) , row_number () over (partition by Year(DTE) order by SUM(quantity) desc) AS ROW_NO 
from fact_transaction a
join Dim_model b on a.IDModel = b.IDModel 
where Year(DTE) IN ( '2008', '2009', '2010' ) 
group by Year(DTE) , Model_name 
 )

select * from xyz where Row_no <=5 ;
...
Select Idmanufacturer , Sum_Qty ,Year_S   from (
select Idmanufacturer , year(DTE) as Year_S, SUM(quantity) as Sum_Qty , Row_number() over (partition by year(DTE) order by SUM(quantity)desc )  as ROW_1
from fact_transaction a
join Dim_model b on a.IDModel = b.IDModel 
group by  Idmanufacturer,  year(DTE) ) xyz
where Row_1 = 2 and Year_S IN ('2010','2009') ;


-- 9. Show the manufacturers that sold cellphones in 2010 but did not in 2009. 

select Distinct(Manufacturer_Name)  from fact_transaction a
inner join dim_model b on a.IDModel = b.IDModel 
inner join dim_manufacturer c on b.IDManufacturer = c.IDManufacturer 
where Year(DTE) = '2010' AND Manufacturer_Name 
NOT IN (
select Distinct(Manufacturer_Name)  from fact_transaction a
inner join dim_model b on a.IDModel = b.IDModel 
inner join dim_manufacturer c on b.IDManufacturer = c.IDManufacturer 
where Year(DTE) = '2009' ) ; -- ALl manufacturers who sold phones in 2010 sol phones in 2009 as well  
...
Select * , (spend - datalag)/(spend) * 100 as percentage_change  from 
(
select * , lag(spend) over ( order by idcustomer , DTE ) datalag 
from
(select idcustomer , avg(totalprice) as Spend , avg(quantity) as Qnt , Year(DTE) as DTE from fact_transaction
group by idcustomer ,  Year(DTE) 
order by year(dte) asc , idcustomer desc limit 100 )xyz ) abc ;
